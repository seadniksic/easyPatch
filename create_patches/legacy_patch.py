...
def generate_patches(image_path: str, output_path: str, patch_size: int, overlap=0, level=0):

    if not os.path.exists(os.path.join(output_path, "patches.hdf5")):
        output_file = h5py.File(os.path.join(output_path, "patches.hdf5"), "w")
    else: 
        output_file = h5py.File(os.path.join(output_path, "patches.hdf5"), "r")

    patch_without_mask(os.path.join(IMAGE_PATH, f'{IMAGE_PATH[]}.svs'), f, LEVEL, PATCH_SIZE, overlap)

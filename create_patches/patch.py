...
def patch_image_without_mask(WSI_NAME, WSI_PATH, LEVEL, PATCH_SIZE, overlap):
    now = time.time()
    print(os.path.join(WSI_PATH, f'{WSI_NAME}.svs')) 
    patched_image, metadata = patch_without_mask(os.path.join(WSI_PATH, f'{WSI_NAME}.svs'), LEVEL, PATCH_SIZE, overlap)
...
'''
def patch_without_mask(image, patch_level, patch_size, overlap_level):        
    ...
    #This will be return tensor (4 at the end since openslide returns 4 channels)
    patched_image = np.empty([num_x_patches * num_y_patches, patch_size, patch_size, 4], dtype=np.uint8)
    ...
    for i in tqdm(range(num_y_patches)):
        for j in range(num_x_patches):
            #Read patch and assign it to array that will be returned
            patched_image[patch_count] = np.array(im.read_region((*anchor,), patch_level, (patch_size, patch_size)))              
            patch_count += 1
            #shift reading frame to the right - im.level_downsamples used to upsample the shift amount since top left coord is based on level 0
            anchor[0] += round(im.level_downsamples[patch_level]) * (patch_size - overlap_x)
    ...
    return patched_image, [overlap_x, overlap_y, num_x_patches, num_y_patches] #, im.get_thumbnail(im.level_dimensions[3]) 
